...
exports.create = async (req, res, next) => {
  if (!req.body?.MSNV || !req.body?.HoTenNV) {
    return next(new ApiError(400, "MSNV và HoTenNV không được để trống"));
  }

  try {
    const nvService = new NhanVienService(MongoDB.client);
    const document = await nvService.create(req.body);
...
// exports.findAll = async (req, res, next) => {
//   if (req.user.Chucvu !== "Admin") {
//     return next(new ApiError(403, "Chỉ Admin mới có quyền xem danh sách nhân viên"));
//   }
...
//       return next(new ApiError(403, "Bạn không có quyền xem thông tin nhân viên khác"));
//     }
...
//     if (req.user.Chucvu !== "Admin" && req.user.MSNV !== req.params.id) {
//       return next(new ApiError(403, "Bạn không có quyền cập nhật nhân viên khác"));
//     }
...
//     return next(new ApiError(403, "Chỉ Admin mới có quyền xóa tất cả nhân viên"));
//   }
